...
[~, sort_idx] = sort(DC_data(:, 2), 'descend'); % 获取排序索引
sorted_DC_data = DC_data(sort_idx, :);

% 提取排序后的数据列
depth = sorted_DC_data(:, 2);
ccarb = sorted_DC_data(:, 3);
corg = sorted_DC_data(:, 4);
delta = sorted_DC_data(:, 5);
...
% 定义颜色
color1 = [66/255, 52/255, 88/255];      % 1元人民币绿色系
color2 = [119/255, 96/255, 142/255];   % 1元人民币绿色系
color3 = [161/255, 142/255, 174/255];   % 1元人民币绿色系
...
xlabel('Delta');
ylabel('Depth');
title('DC: Depth vs Delta');
grid on;

% 调整图窗布局，使其更美观
sgtitle('DC Carbon Data Analysis'); % 设置整个图窗的标题